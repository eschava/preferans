...
// Every game is created by somebody, with their settings; the seats they do not
// fill are played by bots until a person takes them.
...
// The view a seat is allowed to see, plus who at this table is a person.
const viewOf = (r, seat) => ({ ...viewFor(r.game, seat), humans: r.human.slice() });
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/room' && req.method === 'POST') {
    ...
    return json(res, 200, { room: code, seat: 0, token: take(r, 0) });
  }
